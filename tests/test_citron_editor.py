...
def test_65_destructive_range_rejects_missing_bound():
    """delete: 実在しない境界IDを拒否する（行IDの打ち間違いが別行の削除として成功しない）。
    "1i2" は存在しないが、ソート上 "1i1" と "2" の間なので、以前は range 指定として通り
    行2〜3が消えていた。"""
    create_test_file()  # line1..line5
    citron.cmd_open({"file": "test.md"})
    citron.cmd_insert({"after": "1", "content": "inserted"})  # → 1i1 ができる
    r = citron.cmd_delete({"start": "1i2", "end": "3"})       # 1i2 は存在しない
    assert r["status"] == "error", f"存在しない境界IDでdeleteが成立した: {r}"
    assert "1i2" in r["message"]
    # 巻き込み削除が起きていないこと
    v = citron.cmd_view({})
    assert "2: line2" in v["data"]["content"], v["data"]["content"]
    assert "3: line3" in v["data"]["content"], v["data"]["content"]
    citron.cmd_discard({})

def test_66_replace_move_copy_reject_missing_bound():
    """replace/move/copy も範囲境界の実在を要求する（deleteと同じ扱い）"""
    create_test_file()
    citron.cmd_open({"file": "test.md"})
    cases = [
        (citron.cmd_replace, {"search": "line", "replacement": "x", "start": "1i2", "end": "3"}),
        (citron.cmd_move, {"start": "1i2", "end": "3", "after": "5"}),
        (citron.cmd_copy, {"start": "1", "end": "9i9", "after": "5"}),
    ]
    for fn, args in cases:
        r = fn(args)
        assert r["status"] == "error", f"{fn.__name__} が存在しない境界を受理した: {r}"
        assert "view" in r["message"], f"{fn.__name__} のエラーに次の一手が無い: {r['message']}"
    citron.cmd_discard({})

def test_67_deleted_line_still_valid_bound():
    """境界として tombstone（削除済み行）は許可する（仕様。undo後や削除済みを含む再指定のため）"""
    create_test_file()
    citron.cmd_open({"file": "test.md"})
    citron.cmd_delete({"start": "2"})               # 行2をtombstone化
    r = citron.cmd_delete({"start": "2", "end": "3"})  # 境界に削除済み行を含む
    assert r["status"] == "success", f"tombstoneを境界に使えなかった: {r}"
    assert r["data"]["deleted_lines"] == ["3"], r["data"]
    citron.cmd_discard({})

def test_68_diff_shows_before_and_after():
    """diff: 変更前後・削除・挿入が差分記号つきで出る"""
    create_test_file()  # line1..line5
    citron.cmd_open({"file": "test.md"})
    citron.cmd_rewrite({"line": "2", "content": "CHANGED"})
    citron.cmd_delete({"start": "4"})
    citron.cmd_insert({"after": "5", "content": "NEW"})
    r = citron.cmd_diff({})
    assert r["status"] == "success", r
    assert r["data"]["has_changes"] is True
    c = r["data"]["content"]
    assert "- 2: line2" in c, c      # 変更前
    assert "+ 2: CHANGED" in c, c    # 変更後
    assert "- 4: line4" in c, c      # 削除
    assert "+ 5i1: NEW" in c, c      # 挿入
    assert "  1: line1" in c, c      # 文脈行（既定context=2）
    assert r["data"]["changes"] == {"modified": 1, "deleted": 1, "inserted": 1}, r["data"]["changes"]
    # context=0 なら文脈行は出ない
    r0 = citron.cmd_diff({"context": 0})
    assert "  1: line1" not in r0["data"]["content"], r0["data"]["content"]
    assert "+ 2: CHANGED" in r0["data"]["content"]
    citron.cmd_discard({})

def test_69_diff_no_changes():
    """diff: 変更が無ければ has_changes=False（commit前の確認で誤解しないため）"""
    create_test_file()
    citron.cmd_open({"file": "test.md"})
    r = citron.cmd_diff({})
    assert r["status"] == "success", r
    assert r["data"]["has_changes"] is False, r["data"]
    assert r["data"]["content"] == ""
    # 挿入してから削除した行は実ファイルに現れないので差分にも出さない（count_changesと同じ扱い）
    citron.cmd_insert({"after": "1", "content": "temp"})
    citron.cmd_delete({"start": "1i1"})
    r2 = citron.cmd_diff({})
    assert r2["data"]["has_changes"] is False, f"insert→deleteが差分に残った: {r2['data']['content']}"
    citron.cmd_discard({})

def test_70_diff_new_file_shows_all_lines():
    """diff: 新規作成セッションでは全行が追加として出る（元ファイルが無いため）"""
    citron.cmd_open({"file": "brand_new.md", "create": True, "content": "alpha\nbeta"})
    r = citron.cmd_diff({})
    assert r["status"] == "success", r
    assert r["data"]["new_file"] is True, r["data"]
    c = r["data"]["content"]
    assert "+ 1: alpha" in c, c
    assert "+ 2: beta" in c, c
    citron.cmd_discard({})

def test_71_diff_range_and_invalid_context():
    """diff: start/endで範囲を絞れる。contextが数値でなければ自己回復エラー"""
    create_test_file()
    citron.cmd_open({"file": "test.md"})
    citron.cmd_rewrite({"line": "1", "content": "TOP"})
    citron.cmd_rewrite({"line": "5", "content": "BOTTOM"})
    r = citron.cmd_diff({"start": "4", "end": "5", "context": 0})
    c = r["data"]["content"]
    assert "+ 5: BOTTOM" in c, c
    assert "TOP" not in c, f"範囲外の変更が出た: {c}"
    bad = citron.cmd_diff({"context": "たくさん"})
    assert bad["status"] == "error"
    assert "diff" in bad["message"], bad["message"]
    citron.cmd_discard({})

def test_72_commit_incomplete_session_friendly_error():
    """commit: 旧形式でファイル情報キーが欠けたセッションは内部エラーにせずdiscardへ誘導する"""
    create_test_file()
    citron.cmd_open({"file": "test.md"})
    citron.cmd_rewrite({"line": "1", "content": "x"})
    # 旧バージョンの残留セッションを再現（original_file_path が無い）
    sp = citron.get_session_path()
    with open(sp, encoding="utf-8") as f:
        data = json.load(f)
    del data["original_file_path"]
    with open(sp, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False)
    r = citron.cmd_commit({})
    assert r["status"] == "error", f"不完全セッションのcommitが通った: {r}"
    assert "discard" in r["message"], r["message"]
    # status は表示専用なので落ちない
    s = citron.cmd_status({})
...
tests = [
    ...
    ("test_65 delete: 存在しない境界IDを拒否", test_65_destructive_range_rejects_missing_bound),
    ("test_66 replace/move/copy: 境界ID実在チェック", test_66_replace_move_copy_reject_missing_bound),
    ("test_67 境界: tombstoneは許可", test_67_deleted_line_still_valid_bound),
    ("test_68 diff: 変更前後の表示", test_68_diff_shows_before_and_after),
    ("test_69 diff: 変更なし", test_69_diff_no_changes),
    ("test_70 diff: 新規ファイルは全行追加", test_70_diff_new_file_shows_all_lines),
    ("test_71 diff: 範囲指定とcontext不正", test_71_diff_range_and_invalid_context),
    ("test_72 commit: 不完全セッションで誘導", test_72_commit_incomplete_session_friendly_error),
]
