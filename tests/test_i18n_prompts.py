...
検証内容:
  ...
  5. has_unfulfilled_declaration() が言語別に正しく動く（ja は AND 合致、en は隣接 regex、他は False）。
...
def main() -> int:
    ...
    # ----- 各プレフィックスの存在チェック -----
    for prefix in ("tool_", "ctx_", "web_", "rep_", "wyrd_", "salia_", "agent_", "wx_", "img_"):
    ...
    for prefix in ("web_", "rep_", "wyrd_", "salia_", "agent_", "wx_", "img_"):
    ...
    # ----- 5. has_unfulfilled_declaration の言語別動作 -----
    from core.agent import has_unfulfilled_declaration
    # en: 意志マーカー＋動作動詞の隣接で True
    init_i18n({"language": "en"})
    EN_TRUE_CASES = [
        "I'll update self_memo later.",
        "I will write a note about it.",
        "Let me check the latest entries.",
        "I'm going to record this in the journal.",
        "I want to investigate that further.",
        "I should review the previous notes.",
        "I'll just take a look at the dashboard.",
        "Let's visit OpenBotCity tomorrow.",
    ]
    EN_FALSE_CASES = [
        "I want to see you soon.",          # 純感情、動詞リスト外
        "I should be careful here.",         # be は動詞リスト外
        "Let me think about it for a bit.",  # think は動詞リスト外
        "That was a great day.",             # 意志マーカー無し
        "雑記帳に書いてみようと思います。",      # 英語版は日本語入力に反応しない
    ]
    for s in EN_TRUE_CASES:
        if not has_unfulfilled_declaration(s):
            failures.append(f"[en/declaration] True 期待だが False: {s!r}")
    for s in EN_FALSE_CASES:
        if has_unfulfilled_declaration(s):
            failures.append(f"[en/declaration] False 期待だが True: {s!r}")
    # ja: 既存の AND 合致が壊れていないこと
    init_i18n({"language": "ja"})
    if not has_unfulfilled_declaration("雑記帳に書いてみようと思います。"):
        failures.append("[ja/declaration] ja で True が返らない（regex 破損?）")
    if has_unfulfilled_declaration("I'll update self_memo later."):
        # ja モードでは英語入力には反応しない（日本語パターンに掠らない）
        failures.append("[ja/declaration] ja で英語入力に True（パターン汚染?）")
    ...
    print("PASS: tools=%d, ctx_keys=%d, referenced_keys=%d, ja/en matched, no unresolved/leak, "
          "declaration ja/en patterns OK, Layer0 markers intact"
