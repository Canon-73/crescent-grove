...
G. 範囲指定の境界は「破壊的コマンドだけ」実在を必須にする（check_range_bounds）。
   delete/replace/move/copy は start/end が実在しないと弾く。表示系(view/search)は読み取り専用なので
   仮想的な境界（実在しない "10i99" 等）を許したままにしてある。tombstone は lines に残るので
   境界として有効。非対称なのは意図的で、行IDの打ち間違いが別の行の破壊として成功しないため。
============================================================================
"""
import copy
import hashlib
...
# except は ImportError だけでは足りない: get_encoding() はBPEデータが未キャッシュだと
# ネットワークへ取りに行き、オフライン/プロキシ遮断環境では ImportError ではない例外を投げる。
# ここは module import 時に走るので、取りこぼすと main() に到達する前にプロセスごと死に、
# フレームワークには「何も出力しませんでした (exit code: 1)」としか見えない（配布版で実害あり）。
try:
    import tiktoken
    _ENC = tiktoken.get_encoding("cl100k_base")
    def count_tokens(text: str) -> int:
        # disallowed_special=(): 本文に "<|endoftext|>" 等の特殊トークンが含まれていても
        # ValueError にせず、普通の文字列としてトークン数を数える（AI生成の小説等で現実に起きる）
...
# 即KeyError/TypeError（=内部エラー）になるものだけに絞る。file_path/original_file_path/
# file_hash/is_new 等の「ファイル情報キー」は、旧バージョンの残留セッションには無いことが
# あるのでここに含めない（含めると正当な旧セッションまで「破損」扱いになる。test_29参照）。
# その代わり、これらを実際に必要とするコマンド（commit）は _missing_file_info() で個別に
# 検証し、欠けていたらdiscard誘導のエラーへ寄せる。表示だけに使う file_path は .get で読む。
# ※ここは「型の表面」しか見ない。line_order と lines の相互整合（存在しないIDが並ぶ等）までは
#   検証しない。セッションを書くのはcitron自身だけという前提（ヘッダ前提3）に立ち、万一壊れても
#   main()のexcept経由で内部エラーとして返る range に留める。
_SESSION_SCHEMA = {
    "lines": dict,
    "line_order": list,
    "undo_stack": list,
...
def _missing_file_info(session: dict) -> str | None:
    """commitが必要とする「ファイル情報キー」のうち、欠けているものの名前を返す（無ければNone）。
    旧バージョンの残留セッションをcommitしようとした時に KeyError（＝復旧手順の無い内部エラー）で
    落ちるのを防ぎ、discard導線に乗せるための検査。"""
    if not session.get("original_file_path"):
        return "original_file_path"
    # 既存ファイルの編集セッションはハッシュ照合に file_hash が要る（新規作成時はNoneが正しい）
    if not session.get("is_new", False) and not session.get("file_hash"):
        return "file_hash"
    return None
...
def check_range_bounds(session: dict, *bounds) -> dict | None:
    """破壊的コマンド（delete/replace/move/copy）の範囲境界が実在する行かを検証する。
    問題なければNone、実在しなければ自己回復用エラーを返す。

    get_lines_in_range() は「実在するか」ではなくソートキーによる範囲抽出なので、
    存在しない境界（例: "12" のつもりの打ち間違い "1i2"）でも、その並び順の間に別の行があれば
    操作が成立してしまう。rewrite/insert が点指定で実在チェックしているのに範囲指定だけ緩いのは
    非対称なので、破壊的コマンドでは境界の実在を必須にする。
    tombstone（削除済み行）は lines に残っているので境界として許可する
    （削除済みを含む範囲の再指定・undo後の再操作が自然に書けるため）。表示系(view/search)は
    読み取り専用なので、これまで通り仮想的な境界を許す。"""
    for b in bounds:
        if b is None:
            continue
        if b not in session["lines"]:
            return error(t("citron_range_bound_missing", line=b, lb="{", rb="}"))
    return None
...
def cmd_replace(args: dict) -> dict:
    ...
    if range_err:
        return range_err
    # 範囲を絞る場合のみ境界の実在を検証する（範囲省略＝ファイル全体は従来どおり）
    bound_err = check_range_bounds(session, start, end)
    if bound_err:
        return bound_err
...
# diffのcontext既定行数。0だと変更行だけが並んで前後関係が読めないので既定で2行付ける。
DEFAULT_DIFF_CONTEXT = 2


def _original_line_map(session: dict) -> dict | None:
    """open時の本文（LF正規化済み）から「元の行ID → 変更前の内容」を作る。
    openは既存ファイルのi行目に行ID str(i) を順に割り当てるので、添字から行IDを復元できる。
    original_contentを持たない旧セッション・新規作成セッションではNoneを返す（呼び出し側でフォールバック）。"""
    raw = session.get("original_content")
    if raw is None:
        return None
    lines = raw.split("\n")
    # openと同じく末尾改行による空行を落として添字を揃える
    if lines and lines[-1] == "":
        lines.pop()
    return {str(i): c for i, c in enumerate(lines, start=1)}


def cmd_diff(args: dict) -> dict:
    """commit前に「実ファイルへ何が書かれるか」を変更前後で確認する。

    statusは件数しか返さないので、LLMが「意図した変更だけになっているか」を確かめるには
    ファイル全体をviewするしかなかった。巨大ファイルでは現実的でないため、変更行とその前後だけを
    差分形式で返す。表示記号は一般的なdiffに合わせる（"- "=変更前/削除、"+ "=変更後/挿入、
    "  "=文脈行）。飛び地は "..." で区切る。"""
    session = load_session()
    if session is None:
        return error(NO_SESSION())
    ...
    # contextはsearchと同じ流儀（文字列も許容・上限クランプ）。省略時は0ではなくDEFAULT_DIFF_CONTEXT。
    raw_context = args.get("context")
    if raw_context is None or raw_context == "":
        context = DEFAULT_DIFF_CONTEXT
    else:
        try:
            context = max(0, int(raw_context))
        except (TypeError, ValueError):
            return error(t("citron_diff_context_not_number", lb="{", rb="}"))
    context = min(context, MAX_SEARCH_CONTEXT)

    counts = count_changes(session)
    is_new = session.get("is_new", False)
    # 読み取り専用なので、view/searchと同じく先頭挿入行(0i*)を取りこぼさない
    target = get_lines_in_range(session, start, end, include_head=True)
    orig_map = _original_line_map(session)

    # 各行を差分の種別に振り分ける
    entries = []  # [(行ID, "same"|"modified"|"deleted"|"inserted"), ...]
    for lid in target:
        info = session["lines"][lid]
        state = info["state"]
        origin = info.get("origin", "original")
        if state == "deleted":
            # 挿入してから削除した行は実ファイルに一度も現れないので差分にも出さない（count_changesと同じ扱い）
            if origin == "original":
                entries.append((lid, "deleted"))
            continue
        if is_new:
            # 新規作成セッションは元ファイルが無いので、残る行すべてが「これから書かれる行」
            entries.append((lid, "inserted"))
        elif state in ("inserted", "modified"):
            entries.append((lid, state))
        else:
            entries.append((lid, "same"))

    changed_idx = [i for i, (_, kind) in enumerate(entries) if kind != "same"]
    if not changed_idx:
        return success(t("citron_diff_no_changes"), {
            "changes": counts,
            "has_changes": False,
            "truncated": False,
            "content": "",
    ...
    for j in sorted(shown_idx):
        if prev is not None and j != prev + 1:
            parts.append("...")
        lid, kind = entries[j]
        current = session["lines"][lid]["content"]
        if kind == "same":
            parts.append(f"  {lid}: {current}")
        elif kind == "inserted":
            parts.append(f"+ {lid}: {current}")
        elif kind == "deleted":
            # 削除行のcontentは（rewrite後に削除した場合）変更後の内容なので、元内容が引けるならそちらを出す
            before = orig_map.get(lid, current) if orig_map is not None else current
            parts.append(f"- {lid}: {before}")
        else:  # modified
            if orig_map is None:
                before = t("citron_diff_before_unavailable")
            else:
                before = orig_map.get(lid, t("citron_diff_before_unavailable"))
            parts.append(f"- {lid}: {before}")
            parts.append(f"+ {lid}: {current}")
        prev = j

    if is_new:
        msg = t("citron_diff_done_new", count=len(entries))
    else:
        msg = t("citron_diff_done", modified=counts["modified"],
                deleted=counts["deleted"], inserted=counts["inserted"])

    content_str, truncated = truncate_for_display("\n".join(parts))
    if truncated:
        content_str += _truncate_notice()
        msg += t("citron_diff_truncated_hint")
...
def cmd_commit(args: dict) -> dict:
    ...
    # 旧バージョンの残留セッションにはファイル情報キーが無いことがある。KeyError（復旧手順の無い
    # 内部エラー）で落とさず、discardして開き直す導線に乗せる。
    missing = _missing_file_info(session)
    if missing:
        return error(t("citron_session_incomplete", key=missing, lb="{", rb="}"))
    ...
    if is_new:
        ...
        if not os.path.exists(abs_path):
            return error(t("citron_commit_original_missing", file_path=file_path))
    ...
    try:
        ...
        return error(t("citron_commit_tmp_prep_failed", file_path=file_path, e=e))
    try:
        ...
        if os.path.exists(tmp_path):
            os.remove(tmp_path)
        return error(t("citron_commit_write_failed", file_path=file_path, e=e))
    except Exception:
        if os.path.exists(tmp_path):
            os.remove(tmp_path)
        raise
    ...
    return {
        ...
        "data": {
            "commands": {
                ...
                "diff": {
                    "description": t("citron_help_diff_desc"),
                    "args": {
                        "context": t("citron_help_diff_arg_context"),
                        "start": t("citron_help_diff_arg_start"),
                        "end": t("citron_help_diff_arg_end"),
                    },
                    "example": {"command": "diff"}
                },
                "commit": {
