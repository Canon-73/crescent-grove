...
def main():
    ...
    try:
        ...
        # disallowed_special=(): "<|endoftext|>" 等を含むファイルでも ValueError にしない
        ...
        # get_encoding() はBPEデータ未キャッシュ+オフライン等で ImportError 以外の例外を投げる。
        # 取りこぼすと無出力 exit 1 でプロセスが死に、LLMに何も伝わらない。
        print(json.dumps({"status": "error", "message": t("token_counter_err_tiktoken_init", e=e)}, ensure_ascii=False))
        return
