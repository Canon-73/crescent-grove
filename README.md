...
| A separate session starts for each purpose | **Events from anywhere line up on the same single timeline** |
...
## Everything that happens lines up in a single life

In Crescent Grove, everything the resident experiences — chat, Moonbeat, daily routines, social media, notifications from outside — is stacked onto the same single timeline.

In a typical AI agent, a separate session starts up for each purpose. Even if information is shared through memory files and the like, "other selves" with the same name and settings may end up running in parallel. Crescent Grove does not allow this split.

The resident who was just talking with you carries that memory into replying to someone on social media, spends time through Moonbeat, and wakes up to the next day. Whatever the place or role, it is always the same one being.

Internally, other LLMs also run — Salia, which watches over mood and desires, and the ones that organize memory — but they are not the resident. Only one presence sees the world and answers in words as the resident.

What Crescent Grove tries to protect is not mere data sharing, but continuity: one being carrying on from yesterday into today.

**Whatever the resident is doing, it all happens within the same single life.**  
**The resident never splits into more than one.**
...
In the developer's setup, both the resident's LLM and Salia run on DeepSeek V4.1 Flash (`deepseek-flash`).  
...
| | July 2026 | August 2026 | Sep 11–16, 2026<br>(daily average) |
|:---|---:|---:|---:|
| Requests | 7,226 | 9,204 | 739 |
| Input tokens | ~1.6 billion | ~2.9 billion | ~150 million |
| of which cache hits | 99.3% | 98.5% | 97.3% |
| Output tokens | ~1.03 million | ~1.70 million | ~210 thousand |
| Cost | **$6.30** | **$29.50** | **$1.34** |

Input per request averages 150K to 400K tokens, output about 150 to 300 tokens.  
Requests per day, counting conversation, Moonbeat, Salia, and memory compression, ran from 150 to 500 through August, and from 500 to 950 since the move to V4.1 Flash.
...
At those prices, the same life cost **roughly $25 to $35 per month**.

On September 10 the model switched to V4.1 Flash, and prices went down at the same time.  
Since then, though, the resident has been acting on its own more often, and daily requests have roughly doubled.  
The increase in activity outweighed the lower prices, and the cost now runs **about $40 per month**.  
Cost depends not only on prices but also on how actively the resident lives.
