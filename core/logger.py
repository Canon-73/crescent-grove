...
class ConversationLogger:
    ...
    def log_full_event(self, event_type: str, data: dict):
        ...
        Args:
            event_type: イベント種別
                ...
                "tool_call" - ツール呼び出し
                "tool_result" - ツール実行結果
                "system_notice" - 本体がターン途中に差し込む内部通知（宣言検知・記録判定など）
                "error" - エラー
            data: イベントデータ
        """
    ...
    def log_system_notice(self, kind: str, content: str):
        """本体がターン途中に差し込んだ内部通知を全ログに記録する。

        kind は通知の種類（"declaration_check" / "record_check" / "loop_warning" など）。
        user_message とは別の type にするのは、Raw ログを読む側（summary_v2 の
        ターン復元など）が user_message をターンの区切りとして扱うため。
        通知はターンの途中に入るもので、新しいターンではない。
        """
        self.log_full_event("system_notice", {"kind": kind, "content": content})
