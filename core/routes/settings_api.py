...
def _update_domain_list(domain: str, *, add: bool) -> None:
    """security.allowed_domains へのドメイン追加/削除の共通処理。変更があれば保存する。"""
    from core.config_loader import save_settings
    config = core_load_config()
...
async def add_security_domain(req: DomainReq):
    ...
    _update_domain_list(domain, add=True)
    return {"status": "success"}
...
async def delete_security_domain(req: DomainReq):
    ...
    _update_domain_list(req.domain.strip().lower(), add=False)
    return {"status": "success"}
...
async def get_general_config():
    ...
    return {
        ...
        "salia": config.get("salia", {
            ...
            "declaration_check": {"enabled": True}
        }),
...
# --- 一般設定のセクション別反映関数群 ---
# update_general_config から順に呼ばれる。設定項目を追加するときは、対応する
# _apply_* を編集（または新設）し、_GENERAL_CONFIG_KEYS にトップレベルキーを足す。

def _apply_profile(config: dict, req: GeneralConfigReq) -> None:
    """プロファイル設定（user/agentの各フィールド）を反映する。"""
...
def _apply_workspace(config: dict, req: GeneralConfigReq) -> None:
    """ワークスペースパスを反映する。"""
    if "workspace" not in config: config["workspace"] = {}
    config["workspace"]["path"] = req.workspace.get("path", "")


def _apply_logs(config: dict, req: GeneralConfigReq) -> None:
    """ログディレクトリを反映する。
...
def _apply_time(config: dict, req: GeneralConfigReq) -> None:
    """タイムゾーン設定を反映する（コンテキストに注入する時刻表示のみに影響・即時反映）。
...
def _apply_memory_compression(config: dict, req: GeneralConfigReq) -> None:
    """記憶圧縮（LETHE）設定を反映する。"""
...
def _apply_letter_and_boot_memories(config: dict, req: GeneralConfigReq) -> None:
    """手紙ファイルパス（ダッシュボード連携）と起動記憶ファイルリストを反映する。"""
...
def _apply_prompt_files(config: dict, req: GeneralConfigReq) -> None:
    """システムプロンプト（コンテキスト最上部）とポストプロンプト（真・最下部）の
    プロンプトファイル群設定を反映する。

    post_prompts.enabled はエキスパート向けトグル。OFF（既定）ならコンテキストに一切入らない。
    """
...
def _apply_tokenizer(config: dict, req: GeneralConfigReq) -> None:
    """トークナイザー設定を反映する（トークン数見積もりに使用。変更は要再起動）。"""
...
def _apply_salia(config: dict, req: GeneralConfigReq) -> None:
    """Salia（情動評価）設定を反映する。

    somatic_marker / evaluate_turn / record_check / declaration_check は settings.json から
    毎ターン直接読まれるため即時反映。salia.enabled（機能全体ON/OFF）のみ初期化時に
    評価されるため要再起動。
    """
    ...
    # evaluate_turn / record_check / declaration_check は enabled フラグのみ持つ同型セクション
    # （record_check=ツール使用後の内省注入、declaration_check=宣言だけ未実行の再ターン促し。
    #   どちらも core/agent.py の _read_salia_flag が毎ターン読む）
    for key in ("evaluate_turn", "record_check", "declaration_check"):
        if key not in req.salia:
            # 画面が送ってこないキーは現状維持。項目を知らない古い画面（再起動前に
            # キャッシュされたページなど）からの保存で、settings.json に手書きした
            # OFF を勝手に ON へ戻さないため
            continue
        sub_req = req.salia.get(key) or {}
        sub_cfg = config["salia"].get(key, {})
        if not isinstance(sub_cfg, dict): sub_cfg = {}
        sub_cfg["enabled"] = bool(sub_req.get("enabled", True))
        config["salia"][key] = sub_cfg


def _apply_search(config: dict, req: GeneralConfigReq) -> None:
    """Web検索プロバイダー設定を反映する。
...
# 一般設定として settings.json に保存するトップレベルキー
_GENERAL_CONFIG_KEYS = [
    "profile", "workspace", "logs", "time", "memory_compression",
    "letter_file", "boot_memories", "system_prompts", "post_prompts",
    "tokenizer", "salia", "search",
]


@router.post("/api/settings/general")
async def update_general_config(req: GeneralConfigReq):
    """一般設定を更新してsettings.jsonに保存する。反映処理はセクション別の _apply_* に分割してある。"""
    from core.config_loader import save_settings
    config = core_load_config()

    _apply_profile(config, req)
    _apply_workspace(config, req)
    _apply_logs(config, req)
    _apply_time(config, req)
    _apply_memory_compression(config, req)
    _apply_letter_and_boot_memories(config, req)
    _apply_prompt_files(config, req)
    _apply_tokenizer(config, req)
    _apply_salia(config, req)
    _apply_search(config, req)

    save_settings({key: config[key] for key in _GENERAL_CONFIG_KEYS})
