...
async def get_memory():
    ...
    if schedule_path.exists():
        try:
            ...
            result["schedules"] = []
    # --- 現在のトークン使用状況と内訳（リロード直後でも体調タブに表示できるよう同梱） ---
    # 内訳（system / tools / raw / layer0 / layer1 / layer2 / summary_view）は
    # context.get_token_breakdown() の実測値で、UI 側では一切引き算しない。
    # 以前はデバッグ用スナップショットの system ロール合計を System としていたが、
    # それには会話要約（Layer1/2）の system メッセージも含まれていて、
    # UI が逆算していた Raw が Layer1+Layer2 ぶん小さく出ていた。
    ...
    if agent is not None and getattr(agent, "context", None):
        try:
            ...
            # 旧クライアント互換キー。値は token_usage と同じ実測値
            result["context_tokens"] = {
                "system": token_usage.get("system"),
                "tools": token_usage.get("tools"),
            }
        except Exception:
            pass
    return result
