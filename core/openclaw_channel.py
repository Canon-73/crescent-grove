...
class OpenClawChannel:
    ...
    async def run(self):
        ...
        self._running = True
        # HTTP heartbeatループは現状 next_heartbeat_interval しか使っておらず
        # オンライン状態維持はWebSocket側で足りているため停止中。
        # 将来「OBCの世界状況を柚月に自動で差し込みたい」等の用途で復活させる
        # 可能性があるのでメソッド本体(_heartbeat_loop)は残してある。
