...
配布レイアウト（前段 build_runtime.py で確定した想定どおり）:
    ...
    │   └─ python313._pth       "python313.zip / . / .. / ..\programs" + import site（絶対パス無し）
    ├─ server.py               本体エントリ
...
"..\\programs" はサテライトの `from _i18n import t` 用。embeddable Python は ._pth がある
と環境変数 PYTHONPATH を完全に無視するため、dev で使っている _run_program の
PYTHONPATH 注入（core/tools.py）が配布版では効かない。._pth に載せて sys.path に通す。
...
# scripts/ は開発機専用のもの（バックアップ番犬・OSS 公開・柚月のデータ移行）が大半なので
# フォルダごとは同梱せず、利用者が使う道具だけ名前で指定して staging の scripts/ に置く。
# 置き場所を dev と同じ scripts/ にするのは、DISTRIBUTION.md の手順を dev と配布で共通にするため。
INCLUDE_SCRIPTS = ["scripts/build_layer1_from_logs.py"]
...
# この環境固有の同梱除外 (scripts/private_excludes.py) があれば取り込む。
# 何を外したかがビルドスクリプト自体から読めないよう、名前はここに書かない。
# OSS 利用者の手元には存在しないので、その場合は素の挙動で動く。
_private = Path(__file__).resolve().parent / "private_excludes.py"
if _private.exists():
    import importlib.util as _ilu
    _spec = _ilu.spec_from_file_location("_private_excludes", _private)
    if _spec and _spec.loader:
        _mod = _ilu.module_from_spec(_spec)
        _spec.loader.exec_module(_mod)
        _IGNORE_NAMES += list(getattr(_mod, "DIST_IGNORE_NAMES", []))
...
def step_copy() -> None:
    ...
    for f in INCLUDE_FILES:
        ...
        log(f"  {f} をコピー")
    _copy_include_scripts(STAGING)
    # 念のため: 個人データ/秘密が紛れ込んでいないか検査
    _assert_no_secrets()


def _copy_include_scripts(staging: Path) -> None:
    """利用者向けの道具（INCLUDE_SCRIPTS）だけを staging/scripts/ に置く。"""
    for rel in INCLUDE_SCRIPTS:
        src = REPO_ROOT / rel
        if not src.exists():
            fail(f"INCLUDE_SCRIPTS の {rel} が見つかりません")
        dst = staging / rel
...
def _normalize_pth(pth: Path) -> None:
    """._pth を配布用（python313.zip / . / .. / ..\\programs + import site）に正規化。絶対パス行を除去。

    "..\\programs" はサテライトの `from _i18n import t` を配布版で成立させるために必須。
    embeddable Python は ._pth が存在すると環境変数 PYTHONPATH を無視する仕様のため、
    _run_program（core/tools.py）の PYTHONPATH 注入は配布 runtime には届かない。
    この行が無いと全サテライトが起動時 ModuleNotFoundError: '_i18n' で即死する
    （フレームワークからは「何も出力しませんでした exit code: 1」に見える）。"""
    lines = ["python313.zip", ".", "..", "..\\programs", "",
...
def step_check_pth() -> None:
    ...
    if "..\\programs" not in content.splitlines():
        fail("._pth に '..\\programs'（サテライト i18n 用）がありません。"
             "embeddable は PYTHONPATH を無視するため、この行が無いと全サテライトが _i18n の ImportError で即死する")
    log("  OK: 配布用 ._pth（.. で install_root、..\\programs でサテライト i18n、絶対パス無し）")
...
def main() -> None:
    ...
    log(f"同梱             : {', '.join(INCLUDE_DIRS + INCLUDE_FILES + INCLUDE_SCRIPTS + ['runtime/'])}")
